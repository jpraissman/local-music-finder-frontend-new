...
export const dayjsDateSchema = z.custom<dayjs.Dayjs>(
  (date) => {
    return (
      dayjs.isDayjs(date) &&
      date.isValid() &&
...
export const dayjsTimeSchema = z.custom<dayjs.Dayjs>(
  (date) => {
...
export const facebookUrlSchema = z.union([
  z.literal(""),
  z.string().regex(/^(https?:\/\/)?([a-zA-Z0-9-]+\.)*facebook\.com\/.+$/, {
...
export const instagramUrlSchema = z.union([
  z.literal(""),
  z.string().regex(/^(https?:\/\/)?([a-zA-Z0-9-]+\.)*instagram\.com\/.+$/, {
